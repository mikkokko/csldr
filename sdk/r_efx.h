UNIMPLEMENTED_TYPE(particle_t);
UNIMPLEMENTED_TYPE(BEAM);
UNIMPLEMENTED_TYPE(dlight_t);
UNIMPLEMENTED_TYPE(pmtrace_t);
...
typedef struct tempent_s
{
	...
	vec3_t tentOffset;
	cl_entity_t entity;
} TEMPENTITY, tempent_t; /* mikkotodo 2 typedefs for same type? BAH */
...
typedef struct
{
	...
	void (*R_Projectile)(float *, float *, int, int, int, void (*)(tempent_t *, pmtrace_t *));
	...
	TEMPENTITY *(*R_TempSprite)(float *, float *, float, int, int, int, float,
								float, int);
	int (*Draw_DecalIndex)(int);
	int (*Draw_DecalIndexFromName)(const char *);
	void (*R_DecalShoot)(int, int, int, float *, int);
	void (*R_AttachTentToPlayer)(int, int, float, float);
	void (*R_KillAttachedTents)(int);
	BEAM *(*R_BeamCirclePoints)(int, float *, float *, int, float, float, float,
								float, float, int, float, float, float,
								float);
	BEAM *(*R_BeamEntPoint)(int, float *, int, float, float, float, float,
							float, int, float, float, float, float);
	BEAM *(*R_BeamEnts)(int, int, int, float, float, float, float, float, int,
						float, float, float, float);
	BEAM *(*R_BeamFollow)(int, int, float, float, float, float, float, float);
	void (*R_BeamKill)(int);
	BEAM *(*R_BeamLightning)(float *, float *, int, float, float, float, float,
							 float);
	BEAM *(*R_BeamPoints)(float *, float *, int, float, float, float, float,
						  float, int, float, float, float, float);
	BEAM *(*R_BeamRing)(int, int, int, float, float, float, float, float, int,
						float, float, float, float);
	...
	TEMPENTITY *(*CL_TentEntAllocCustom)(float *, model_t *, int,
										 void (*)(tempent_t *, float, float));
